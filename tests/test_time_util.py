...
def test_to_offset_aware():
    ...
    aware_dt = lm.to_offset_aware(NAIVE_DT, offset_seconds)
    assert aware_dt.tzinfo is not None
    assert aware_dt.tzinfo == offset_zone

    # Test with already aware datetime
    aware_dt_from_utc = lm.to_offset_aware(UTC_AWARE_DT, offset_seconds)
...
def test_to_utc_aware():
    # Test with naive datetime
    utc_dt = lm.to_utc_aware(NAIVE_DT)
    ...
    utc_dt_from_aware = lm.to_utc_aware(aware_dt)
...
def test_to_eastern_aware_handles_dst():
    ...
    eastern_dt_winter = lm.to_eastern_aware(winter_naive)
    assert eastern_dt_winter.tzinfo == lm.EASTERN_TIMEZONE
    ...
    eastern_dt_summer = lm.to_eastern_aware(summer_naive)
    assert eastern_dt_summer.tzinfo == lm.EASTERN_TIMEZONE
    ...
    eastern_from_aware = lm.to_eastern_aware(utc_aware_summer)
...
def test_get_current_event_time():
    current_event_time = lm.get_current_event_time()
    assert current_event_time.tzinfo is not None
    assert current_event_time.tzinfo == lm.EASTERN_TIMEZONE
