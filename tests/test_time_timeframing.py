...
@pytest.mark.parametrize(
    ...
    [
        ("hourly"   , lm.HourlyFrame    , lm.TimeFrameResolution.hourly     ),
        ("daily"    , lm.DailyFrame     , lm.TimeFrameResolution.daily      ),
        ("weekly"   , lm.WeeklyFrame    , lm.TimeFrameResolution.weekly     ),
        ("monthly"  , lm.MonthlyFrame   , lm.TimeFrameResolution.monthly    ),
        ("quarterly", lm.QuarterlyFrame , lm.TimeFrameResolution.quarterly  ),
        ("yearly"   , lm.YearlyFrame    , lm.TimeFrameResolution.yearly     ),
        ("none"     , lm.TimeFrame      , lm.TimeFrameResolution.none       ),
    ],
)
def test_time_frame_resolution_mapping(key, frame_type, resolution):
    ...
    assert lm.TimeFrameResolution.from_name(key) == resolution
    assert lm.TimeFrameResolution.from_type(frame_type) == resolution


def test_time_frame_resolution_from_name_invalid():
    """Tests the default case for from_name with an invalid name."""
    assert (
        lm.TimeFrameResolution.from_name("invalid", default=lm.TimeFrameResolution.none)
        == lm.TimeFrameResolution.none
    )
...
class TestTimeFrame:
    def test_init_with_floor_and_ceiling(self):
        ...
        frame = lm.TimeFrame.create(floor=floor, ceiling=ceiling)
    ...
    def test_init_with_moment(self):
        ...
        frame = lm.TimeFrame.create(moment=MOMENT)
    ...
    def test_get_floor_ceiling_with_offset(self):
        frame = lm.DailyFrame.create(moment=MOMENT, tzone=PST)
...
class TestTimeFrames:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                lm.WeeklyFrame,  # Week starts on Monday, moment is a Thursday
                MOMENT,
            ...
            (
                lm.QuarterlyFrame,  # October is in Q4
                MOMENT,
                datetime(2023, 10, 1, 0, 0, 0, tzinfo=timezone.utc),
...
@pytest.mark.parametrize(
    "frame_class",
    [lm.YearlyFrame, lm.QuarterlyFrame, lm.MonthlyFrame, lm.WeeklyFrame, lm.DailyFrame, lm.HourlyFrame],
)
def test_align_to_human_timeframe_standard(frame_class):
    """Tests aligning a standard frame finds the correct type."""
    frame = frame_class.create(moment=MOMENT)
    aligned_frame = lm.align_to_human_timeframe(frame.floor, frame.ceiling, frame.alignment_offset_seconds)
...
def test_align_to_human_timeframe_offset():
    ...
    monthly_pst = lm.MonthlyFrame.create(moment=MOMENT, tzone=PST)
    aligned_monthly = lm.align_to_human_timeframe(
        monthly_pst.floor, monthly_pst.ceiling, monthly_pst.alignment_offset_seconds
    )
    assert isinstance(aligned_monthly, lm.MonthlyFrame)
    assert (
...
def test_align_to_human_timeframe_non_standard():
    ...
    non_standard = lm.align_to_human_timeframe(floor, ceiling)
    assert type(non_standard) is lm.TimeFrame
...
def test_time_status(
    ...
    frame = lm.HourlyFrame.create(moment=MOMENT)
